...
slope, intercept, r_value, p_value, std_err = linregress(m, u)
line = slope * m + intercept

# 绘制散点图和拟合直线
plt.scatter(m, u, color='blue', label='Data points')
plt.plot(m, line, color='red', label=f'Fit line: U = {slope:.5f}f + {intercept:.5f}')
...
# 显示相关系数
plt.text(min(m), max(u)-15, f'r = {r_value:.5f}', fontsize=12, color='green')
